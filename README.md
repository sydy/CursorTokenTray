...
1. 在一台电脑打开 **设置 → 同步**，用邮箱注册（无需验证）
2. 在另一台电脑用**同一邮箱和密码**登录
3. 启动时、保存账号 / 设置时、定时刷新时会自动与云端合并：按账号 `id` 最后写入获胜；删除会留下墓碑。配置（刷新间隔、告警、通知、图标、月费、汇率）按整份最后写入获胜
4. 加密信封仍是 PBKDF2 + AES-256-GCM，密钥由登录密码在本地派生。本机 `config.json` 里的登录态按操作系统加密保存

只同步账号身份和上述跨设备设置，不覆盖开机自启、告警去重、用量历史和报表缓存。  
